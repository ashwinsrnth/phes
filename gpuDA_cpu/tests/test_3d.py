...
class TestGpuDA3d:
    ...
    def test_gtol(self):
        ...
        if self.rank == 13:
            assert(np.all(b[1:-1,1:-1,0] == 12))
            assert(np.all(b[1:-1,1:-1,-1] == 14))
            assert(np.all(b[1:-1,0,1:-1] == 10))
            assert(np.all(b[1:-1,-1,1:-1] == 16))
            assert(np.all(b[0,1:-1,1:-1] == 4))
            assert(np.all(b[-1,1:-1,1:-1] == 22))
    ...
    def test_ltog(self):
        ...
        # a is empty
        a = np.empty([nz,ny,nx], dtype=np.float64)

        self.da.local_to_global(b, a)

        # test ltog:
        if self.rank == 0:
            assert(np.all(a == b[1:-1,1:-1,1:-1]))
